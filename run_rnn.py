...
class Run_AFD_RNN(object):
    ...
    def __init__(self, mode_dir):
        ...
        self.rnn_net = AFD_RNN(net_config, batch_size)
        predict = rnn_net.build_net_graph()
        self.predict_tensor = tf.argmax(predict, axis=2)
    ...
    def run_net(self, data):

        predict = sess.run(self.predict_tensor, feed_dict={self.rnn_net.input_tensor: data})
        return predict
